...
class PrayerAnalyzer(private val context: Context) {
    ...
    fun analyze(bitmap: Bitmap, rotation: Int): PostureResult? {
        ...
        // Copy entire buffer content to local FloatArray to avoid expensive direct ByteBuffer lookups in loop
        val totalElements = channels * numAnchors
        if (cachedOutputArray == null || cachedOutputArray!!.size != totalElements) {
            cachedOutputArray = FloatArray(totalElements)
        }
        val outputArray = cachedOutputArray!!
        ...
        for (i in 0 until numAnchors) {
            ...
            for (c in 0 until numClasses) {
                ...
                val score = outputArray[channelIndex * numAnchors + i]
